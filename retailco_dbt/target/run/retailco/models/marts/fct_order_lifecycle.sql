  ...
  create  table "warehouse"."marts_marts"."fct_order_lifecycle__dbt_tmp"
  
  
    as
  
  (
    with orders as (select * from "warehouse"."marts_staging"."stg_orders"),
dc as (
    select * from "warehouse"."marts_marts"."dim_customer"
    where is_current = true
),
ds as (select * from "warehouse"."marts_marts"."dim_store"),
dd as (select * from "warehouse"."marts_marts"."dim_date")
select
    md5(cast(coalesce(cast(o.order_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT))                                              as lifecycle_sk,
